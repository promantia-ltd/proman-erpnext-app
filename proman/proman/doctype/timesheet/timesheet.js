...
frappe.ui.form.on("Timesheet Detail", "hours",function(frm, doctype, name) {
	set_billing_hours(cur_frm, doctype, name)
});
...
function set_billing_hours(cur_frm,doctype,name){
	...
	row.total_billing_hours=row.hours*row.no_of_workers
	if(row.is_billable==1){
		row.billing_hours=row.hours*row.no_of_workers
	}
