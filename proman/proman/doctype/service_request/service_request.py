...
class ServiceRequest(Document):
	...
	def set_title(self):
		...
		if not self.title:
			...
			self.title = _('Service Request for {1}').format(self.material_request_type, items)[:100]
	...
	def check_modified_date(self):
		mod_db = frappe.db.sql("""select modified from `tabService Request` where name = %s""",
			self.name)
...
def update_status(name, status):
	material_request = frappe.get_doc('Service Request', name)
...
def make_purchase_order_based_on_supplier(source_name, target_doc=None, args=None):
	...
	target_doc = get_mapped_doc("Service Request", mr, {
		"Service Request": {
			"doctype": "Purchase Order",
		},
		"Service Request Item": {
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	service_requests = frappe.db.sql("""select distinct mr.name, transaction_date,company
		from `tabService Request` mr, `tabService Request Item` mr_item
...
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	doc = frappe.get_doc("Service Request", filters.get("doc"))
	item_list = []
	for d in doc.items:
